...
last_matchday = df_past.matchday.max() if not np.isnan(
    df_past.matchday.max()) else 0
...
for matchday in range(1, next_matchday + 1):
    ...
    for _, event in df_event[df_event.matchday == matchday].iterrows():
        ...
        probs = ELO_model.predict(team_vector.reshape(
            1, len(team_ranks))).flatten()

        pred = np.zeros(len(probs))
        pred[np.argmax(probs)] = 1

        # Write predictions to df_event
        df_event.loc[df_event.event_id == event_id,
                     'pred_away_win_prob'] = probs[0]
        df_event.loc[df_event.event_id == event_id, 'pred_tie_prob'] = probs[1]
        df_event.loc[df_event.event_id == event_id,
                     'pred_home_win_prob'] = probs[2]
        ...
        df_event.loc[df_event.event_id == event_id,
                     'briar_score'] = elo_funcs.briar_score(probs, pred)
        ...
        if away_win:
            ...
            new_home_rank = elo_funcs.update_rank_exp(current_home_away_rank,
                                                      goal_differential,
                                                      home_win, probs[0])
            team_ranks[team_to_idx[home]][0] = new_home_rank

            new_away_rank = elo_funcs.update_rank_exp(current_away_away_rank,
                                                      goal_differential,
                                                      away_win, probs[0])
            team_ranks[team_to_idx[away]][0] = new_away_rank

            df_ranks.loc[df_ranks.team == home,
                         f'matchday_{matchday}_away_rank_exp'] = new_home_rank

            df_ranks.loc[df_ranks.team == away,
                         f'matchday_{matchday}_away_rank_exp'] = new_away_rank

            df_ranks.loc[
                df_ranks.team == home,
                f'matchday_{matchday}_tie_rank_exp'] = current_home_tie_rank

            df_ranks.loc[
                df_ranks.team == away,
                f'matchday_{matchday}_tie_rank_exp'] = current_away_tie_rank

            df_ranks.loc[
                df_ranks.team == home,
                f'matchday_{matchday}_home_rank_exp'] = current_home_home_rank

            df_ranks.loc[
                df_ranks.team == away,
                f'matchday_{matchday}_home_rank_exp'] = current_away_home_rank
        ...
        if tie:
            ...
            df_ranks.loc[
                df_ranks.team == home,
                f'matchday_{matchday}_away_rank_exp'] = current_home_away_rank

            df_ranks.loc[
                df_ranks.team == away,
                f'matchday_{matchday}_away_rank_exp'] = current_away_away_rank

            df_ranks.loc[df_ranks.team == home,
                         f'matchday_{matchday}_tie_rank_exp'] = new_home_rank

            df_ranks.loc[df_ranks.team == away,
                         f'matchday_{matchday}_tie_rank_exp'] = new_away_rank
        ...
        if home_win:
            ...
            df_ranks.loc[df_ranks.team == home,
                         f'matchday_{matchday}_home_rank_exp'] = new_home_rank

            df_ranks.loc[df_ranks.team == away,
                         f'matchday_{matchday}_home_rank_exp'] = new_away_rank
...
df_ranks = df_ranks.sort_values(f'matchday_{last_matchday}_rank_exp',
