...
df_CPL_2021 = pd.read_csv(
    "./data/historic/CPL_FBRef_2021.csv",
)
...
X_train, X_test, Y_train, Y_test = ML_funcs.construct_ELO_v2_training_data(
    df_event, N_teams, team_to_idx, N_test_games=100
)
...
# Save the last model, doesn't matter, just want the guts
ELO_model.layers[-1].set_weights([mean_ranks_exp, bias])
ELO_model.save("./elo_model_v2.keras")
