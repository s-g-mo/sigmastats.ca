...
df_CPL_2022 = pd.read_csv(
    "./data/historic/CPL_FBRef_2022.csv",
)
df_CPL_2023 = pd.read_csv(
    "./data/historic/CPL_FBRef_2023.csv",
)
...
data_driven_bias = elo_funcs.convert_prob_to_elo_diff_exp(home_win_rate + tie_rate / 2)
...
df_team = pd.DataFrame.from_dict({v: k for k, v in team_to_idx.items()}, orient="index")
df_team.columns = ["team"]

# Build training/testing
X_train, X_test, Y_train, Y_test = ML_funcs.construct_ELO_v1_training_data(
    df_event, N_teams, team_to_idx, N_test_games=20
)
...
df_elo_2024.rename(
    columns={"mean_learned_rank": "matchday_0_rank_standard"}, inplace=True
)
...
df_elo_2024.to_csv("./data/ranks/ranks_matchday_1.csv")
print(df_elo_2024)
