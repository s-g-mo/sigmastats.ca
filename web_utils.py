...
def beautify_event_table(df_event: pd.DataFrame):
    ...
    df_event[['Home Score', 'Away Score']] = df_event[['Home Score', 'Away Score']].map(lambda x: x.replace('.0', ''))
...
def beautify_rank_table(df_rank: pd.DataFrame, next_matchday: int):
    rank_display_cols = ['team'] + [f'matchday_{i}_rank_standard' for i in range(0, next_matchday)]
    df_rank = df_rank[rank_display_cols]

    rank_col_mapping_1 = {'team': 'Team'}
    rank_col_mapping_2 = {f'matchday_{i}_rank_standard': f'Rank Into Matchday {i+1}' for i in range(0, next_matchday)}
    ...
    df_rank[numeric_columns] = df_rank[numeric_columns].map(lambda x: round(x) if not pd.isna(x) else 0)
